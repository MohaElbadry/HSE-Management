...
<div class="flex h-fit  pt-3 justify-center bg-white text-center text-5xl font-bold shadow-md">
    ...
    <form class="mt-8 space-y-6 bg-gray-100 p-4 border-gray-200 shadow-xl  rounded-xl "
        action="{{ route('incidents.update', $incident->id) }}" method="POST">
        @csrf
        @method('PUT')
        <div>
            <button type='button'
        ...
        <div class="relative">
            ...
            <input
                ...
                name="lib" placeholder="Libellet" value="{{ $incident->lib }}">
        </div>
        <div class="mt-8 content-center">
            ...
            <input
                ...
                name="description" placeholder="Enter The Description" value="{{ $incident->description }}">
        </div>
        <div class="mt-8 content-center">
            ...
            <select class="form-select text-base px-4 py-2 border-b rounded-2xl" name="projet_id">
                ...
                <option value="{{ $item->id }}" {{ $item->id == $incident->projet_id ? 'selected' : '' }}>
                    {{ $item->name }}</option>
                @endforeach
            </select>
        </div>
