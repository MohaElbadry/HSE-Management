...
<div class="flex h-full  pt-3 justify-center bg-white text-center text-5xl font-bold shadow-md">
  <!-- component -->
  <!-- component -->
  <div class="max-w-md py-4 h-1/3 w-2/3 px-8 bg-cyan-300 shadow-lg rounded-lg my-20">

    <div>
      <h2 class="text-gray-900 text-3xl font-semibold">Name : {{ $projet->name }}</h2>
      <p class="mt-2 text-gray-600 text-xl ">description : {{ $projet->description }}
      <p class="mt-2 text-gray-600 text-xl ">USER_ID : {{ $projet->user_id }}
      <p class="mt-2 text-gray-600 text-xl ">SITE_IS : {{ $projet->site_id }}
      </p>
    </div>
    <div class="flex justify-end mt-6">
      <a href="#" class="text-xl font-medium text-indigo-500">Projet start at : {{ $projet->proj_start }}</a>
    </div>
  </div>
</div>
