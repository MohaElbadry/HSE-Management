...
<div class="flex h-full  pt-3 justify-center bg-white text-center text-5xl font-bold shadow-md">
   ...
   <div class="max-w-md py-4 h-1/3 w-fit px-8 bg-cyan-300 shadow-lg rounded-lg my-20">
      ...
      <div>
         <h2 class="text-gray-900 text-3xl font-semibold">Name : {{ $user->name }}</h2>
         <p class="mt-2 text-gray-600 text-xl ">description : {{ $user->email }}
         </p>
      </div>
      <div>
         <p class="mt-2 text-gray-600 text-xl ">ROLE : {{ $user->role }}
         </p>
      </div>
      <div class="flex justify-end mt-6">
         <a href="#" class="text-xl font-medium text-indigo-500">{{ $user->created_at }}</a>
      </div>
   </div>
</div>
