...
`))}))}function ve(h,e,t){let n=S.default.map(e,(r,s)=>S.default.isEmpty(r)||r===t[s]?null:{path:h,line:s+1,original:r,formatted:t[s]});return S.default.without(n,null)}async function ct(h){let e=S.default.without(P,"@switch","@forelse","@php").join("|"),t=new RegExp(`(?!\\/\\*.*?\\*\\/)(${e})(\\s*?)${y}`,"gmi");return new Promise(n=>n(h)).then(n=>S.default.replace(n,t,(r,s,a,l)=>L(`<?php ${s.substr("1")}${a}(${l}) ?>`).replace(/<\?php\s(.*?)(\s*?)\((.*?)\);*\s\?>\n/gs,(o,c,d,p)=>`@${c.trim()}${d}(${p.trim()})`).replace(/([\n\s]*)->([\n\s]*)/gs,"->").replace(/,\)$/,")").replace(/(?:\n\s*)* as(?= (?:&{0,1}\$[\w]+|list|\[\$[\w]+))/g," as"))).then(n=>L(n))}async function be(h){return ct(h)}function Pe(h){let e=S.default.without(T,"@case"),t=S.default.without(M,"@break");return new Promise(n=>n(h)).then(n=>{let r=new RegExp(`(${e.join("|")})([\\s]*?)${y}`,"gis");return S.default.replace(n,r,(s,a,l,o)=>`<beautifyTag start="${a}${l}" exp="^^^${S.default.escape(o)}^^^">`)}).then(n=>{let r=new RegExp(`(?!end=".*)(${t.join("|")})(?!.*")`,"gi");return S.default.replace(n,r,(s,a)=>`</beautifyTag end="${a}">`)})}function $e(h){return new Promise(e=>e(h)).then(e=>S.default.replace(e,/<beautifyTag.*?start="(.*?)".*?exp=".*?\^\^\^(.*?)\^\^\^.*?"\s*>/gs,(t,n,r)=>`${n}(${S.default.unescape(r)})`)).then(e=>S.default.replace(e,/<\/beautifyTag.*?end="(.*?)"\s*>/gs,(t,n)=>`${n}`))}var ht=["/\\*\\* phptag_start \\*\\*/","/\\*\\* end_phptag \\*\\*/","/\\*escaped\\*/","__BLADE__;","/\\* blade_comment_start \\*/","/\\* blade_comment_end \\*/","/\\*\\*\\*script_placeholder\\*\\*\\*/","beautifyTag","@customdirective","@elsecustomdirective","@endcustomdirective"];function xe(h){if(new RegExp(ht.join("|")).test(h))throw new Error(["Can't format blade: something goes wrong.","Please check if template is too complicated or not. Or simplify template might solves issue."].join(`
...
`).length>1?o=L(`<?php${o}?>`,this.defaultPhpFormatOption).trim():o=`<?php${o}?>`,i.default.replace(o,/^(\s*)?<\?php(.*?)\?>/gms,(g,x,b)=>{if(this.isInline(o))return`@php${b}@endphp`;let I=this.preserveStringLiteralInPhp(b),et=this.indentRawBlock(p,I);return`@php${this.restoreStringLiteralInPhp(et)}@endphp`})}),s=i.default.replace(s,/(?<=[\S]+)(\s*?)\/\*\*\*script_placeholder\*\*\*\/(\s)?/gim,(a,l,o)=>{if(o!==void 0)return o;let c=l!=null?l:"",d=o!=null?o:"";return c+d}),s}),t.test(n)&&(n=this.restoreBladeDirectivesInScripts(n)),n}async formatPreservedBladeDirectives(e){return Fe.default.map(e,async t=>{let n=await this.formatAsHtml(t);return(await this.formatAsBlade(n)).trimRight(`
...
`)))}};var le="blade-formatter";var Me="1.32.12";async function Ye(){let h=await(0,Ue.default)((0,Ze.hideBin)(process.argv)).usage(`${W.default.green(le)} ${Me}
